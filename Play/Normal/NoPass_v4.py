...
class Defense(State):
    ...
    @override
    def transFunction(self) -> str:
        if Ball.posX() > 0 and not Enemy.isEnemyControlBall() and Player.isOurPlayerControlBall():
            return "Dribble"
        if not Player.isOurPlayerControlBall():
            return "RushToBall"

class RushToBall(State):
    @override
    def getMatchString(self) -> str:
        return "[A][B]{C}"
